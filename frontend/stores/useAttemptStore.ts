...
import {ApiError,getAttempt,patchAttempt,startSession,submitAttempt} from "@/lib/api";
...
export const useAttemptStore=create<State>((set,get)=>({
  ...
  async submit(){
    ...
    if(get().attemptId===snapshot.attemptId){set({...values(submitted),editGeneration:get().editGeneration});window.dispatchEvent(new CustomEvent("khollelab:progression-refresh",{detail:{reason:"submission"}}))}
  },
  ...
  reset(){loadGeneration++;saveRunnerGeneration++;savePromise=undefined;set({sessionId:undefined,attemptId:undefined,problemId:undefined,status:"draft",solution:"",revision:0,elapsedSeconds:0,saveState:"idle",serverAttempt:undefined,recovery:undefined,editGeneration:0,loadGeneration})}
}));
