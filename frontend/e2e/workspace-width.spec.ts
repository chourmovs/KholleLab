...
for(const viewport of viewports)test(`workspace remains structured at ${viewport.width}px`,async({page})=>{
  ...
  const geometry=await page.evaluate(()=>{
    ...
    const header=box(".app-header"),logo=box(".desktop-brand"),utilities=box(".header-utilities"),professor=box(".professor-pane"),portrait=box(".professor-portrait"),navigation=box(".problem-navigation"),board=box(".blackboard-pane"),tools=box(".tool-rail");
    return {logoContained:contains(header,logo),logoUtilitiesOverlap:intersects(logo,utilities),portraitContained:contains(professor,portrait),navigationPortraitOverlap:intersects(navigation,portrait),toolsContained:contains(board,tools)};
  });
