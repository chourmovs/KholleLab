...
test("touch input and scientific keyboard share the mobile MathLive field",async({browser})=>{
 ...
 const field=page.locator("math-field[aria-label='Tableau de résolution']");await expect(field).toBeVisible();await expect(page.locator(".save")).toContainText("Enregistré");await field.tap();
 ...
 await scientific.tap();await expect(scientific).toHaveAttribute("aria-expanded","false");await field.tap();await expect.poll(()=>field.evaluate(element=>element.shadowRoot?.querySelector("[part=keyboard-sink]")?.getAttribute("inputmode"))).toBe("text");await expect.poll(()=>field.evaluate(element=>(element as HTMLElement&{hasFocus:()=>boolean}).hasFocus())).toBe(true);
 await page.keyboard.type(" donc");await page.keyboard.press("Enter");await page.keyboard.type("f(x)=x+1");await expect.poll(()=>field.evaluate(element=>(element as HTMLElement&{value:string}).value)).toContain("x+1");await expect(page.locator(".save")).toContainText(/Enregistré|Sauvegarde/,{timeout:5000});
