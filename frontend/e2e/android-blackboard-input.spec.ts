...
test("touch input and scientific keyboard share the mobile MathLive field",async({browser})=>{
 ...
 await expect.poll(()=>field.evaluate(element=>element.shadowRoot?.querySelector("[part=keyboard-sink]")?.getAttribute("inputmode"))).toBe("none");expect((await field.evaluate(element=>(element as HTMLElement&{value:string}).value))).toBe(before.value);
 await scientific.tap();await expect(scientific).toHaveAttribute("aria-expanded","false");await field.tap();await expect.poll(()=>field.evaluate(element=>element.shadowRoot?.querySelector("[part=keyboard-sink]")?.getAttribute("inputmode"))).toBe("text");
