...
export interface Evaluation {provider?:string;model?:string;model_family?:string;inference_backend?:string;status:"running"|"completed"|"failed";stage:EvaluationStage;progress:number;elapsed_ms?:number;verdict?:string;score?:number;max_score:number;confidence?:number;strategy_summary?:string;reference_relationship?:string;rubric?:{mathematical_correctness:number;rigor:number;clarity:number;efficiency:number};strengths:string[];issues:MathIssue[];missing_justifications:string[];key_feedback?:string;reference_method_summary?:string;suggested_improvement?:string;error_code?:string;provider_retry_count?:number;next_retry_at?:string}
...
export interface ConsolidationItem {kind:"knowledge"|"topic"|"skill";identifier:string;label:string;state:MasteryState;reasons:string[];prerequisites?:{identifier:string;label:string;status?:"not_practiced"|"not_established"}[]}
...
export interface ProgressionSummary {total_xp:number;grade:number;current_grade_start_xp:number;next_grade_xp:number;xp_to_next_grade:number;current_streak_days:number;longest_streak_days:number;active_today:boolean;last_active_date:string|null;completed_sessions:number;timezone:string}
