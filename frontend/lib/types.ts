...
export interface TutorAssessment {status:"completed";assessment_id:string;revision:number;student_state:string;intervention_needed:boolean;intervention_type:string;intervention:string|null;confidence:number;effective_help_level:number;provider:string;model:string;backend:string}
