...
export function KholleWorkspace({legacyActiveProblemId}:{legacyActiveProblemId?:string}){
  ...
  useEffect(()=>{if(initialized.current)return;initialized.current=true;queueMicrotask(async()=>{getCurriculum().then(setMetadata).catch(()=>{});if(legacyActiveProblemId){setState("loading");try{const restored=await getProblem(legacyActiveProblemId);useAttemptStore.getState().reset();acceptProblem(restored);setState("ready")}catch{setState("error")}}else await loadGuidedProblem()})},[acceptProblem,legacyActiveProblemId,loadGuidedProblem]);
