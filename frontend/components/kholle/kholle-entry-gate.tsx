...
export function KholleEntryGate(){
 const router=useRouter(),[state,setState]=useState<GateState>("loading"),[legacyActiveProblemId,setLegacyActiveProblemId]=useState<string>();
 const check=useCallback(async()=>{setState("loading");setLegacyActiveProblemId(undefined);try{const progress=await getCurriculumProgress();if(progress.onboarding_completed){setState("authorized");return}const active=await getActiveSession();if(active){setLegacyActiveProblemId(active.problem_id);setState("authorized");return}setState("redirecting");router.replace("/onboarding")}catch{setState("error")}},[router]);
 useEffect(()=>{queueMicrotask(()=>void check())},[check]);
 if(state==="authorized")return <KholleWorkspace legacyActiveProblemId={legacyActiveProblemId}/>;
