...
function ProgressionSection({progression}:{progression:ProgressionSummary}) {
  const range=progression.next_grade_xp-progression.current_grade_start_xp;
  const gradeProgress=range?Math.max(0,Math.min(100,(progression.total_xp-progression.current_grade_start_xp)/range*100)):0;
  ...
  return <section className="progression-section" aria-labelledby="progression-title">
    ...
    <div className="progression-stats"><strong>Grade {progression.grade}</strong><span>{progression.total_xp} XP</span><span><Flame aria-hidden="true"/> Série actuelle : {progression.current_streak_days} jour{progression.current_streak_days>1?"s":""}</span><span>Record : {progression.longest_streak_days} jour{progression.longest_streak_days>1?"s":""}</span></div>
    <div className="grade-progress" role="progressbar" aria-label="Progression vers le prochain grade" aria-valuemin={progression.current_grade_start_xp} aria-valuemax={progression.next_grade_xp} aria-valuenow={progression.total_xp}><span style={{width:`${gradeProgress}%`}}/></div><p>{progression.xp_to_next_grade} XP avant le grade {progression.grade+1}</p>
...
function MasteryContent({profile}:{profile:LearnerProfile}) {
  ...
  return <>
    ...
    <section className="mastery-section" aria-labelledby="mastery-title"><h2 id="mastery-title">Maîtrise mathématique</h2><p>La maîtrise par notion est indépendante des XP, du grade et des séries.</p>{profile.knowledge.length===0?<div className="profile-empty"><h3>Pas encore assez de séances pour établir une maîtrise.</h3><p>Cette synthèse se précisera avec des travaux évalués.</p></div>:Object.entries(grouped).map(([domain,nodes])=><section key={domain} className="knowledge-domain"><h3>{domain}</h3><div className="profile-grid">{nodes.map(item=><Area key={item.identifier} item={item}/>)}</div></section>)}</section>
