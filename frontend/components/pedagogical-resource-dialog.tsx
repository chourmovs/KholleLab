...
export function PedagogicalResourceDialog({resourceId,onClose}:{resourceId:string;onClose:()=>void}){
  ...
  useEffect(()=>{let active=true;queueMicrotask(()=>{setResource(undefined);setFailed(false);setSolution(false)});getResource(resourceId).then(value=>{if(active)setResource(value)}).catch(()=>{if(active)setFailed(true)});return()=>{active=false}},[resourceId]);
