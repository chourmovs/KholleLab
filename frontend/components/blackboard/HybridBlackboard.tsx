...
export function HybridBlackboard({ solution, onChange, curriculumLevel }: { solution: string; onChange: (markdown: string) => void; curriculumLevel?: string }) {
  ...
  const refs=useRef(new Map<string,MathEquationEditorHandle>()); const lastSerializedValueRef=useRef(solution);
  ...
  const insert=useCallback((latex:string)=>{if(activeMathBlockId)refs.current.get(activeMathBlockId)?.insert(latex)},[activeMathBlockId]);
