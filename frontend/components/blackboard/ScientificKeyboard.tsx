...
export function ScientificKeyboard({ onInsert, curriculumLevel }: { onInsert: (latex: string) => void; curriculumLevel?: string }) {
  ...
  const toggle=()=>{const next=!open;setOpen(next);if(window.mathVirtualKeyboard)next?window.mathVirtualKeyboard.show():window.mathVirtualKeyboard.hide()};
