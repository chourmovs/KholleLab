...
export const UnifiedBlackboard=forwardRef<UnifiedBlackboardHandle,{solution:string;onChange:(value:string)=>void;readOnly?:boolean}>(function UnifiedBlackboard({solution,onChange,readOnly=false},ref){
 ...
 useImperativeHandle(ref,()=>({toggleKeyboard(){const mf=field.current;if(!mf||locked.current)return;if(window.mathVirtualKeyboard.visible){const position=mf.position;window.mathVirtualKeyboard.hide();setNativeInput(mf,true);/* MathLive may retain a stale virtual-keyboard focus on Android. Re-entering focus moves hardware/native input back to its keyboard sink. */mf.blur();mf.focus();mf.position=position;return}setNativeInput(mf,false);mf.focus();window.mathVirtualKeyboard.show();},insertLineBreak,insertText}),[insertLineBreak,insertText,setNativeInput]);
