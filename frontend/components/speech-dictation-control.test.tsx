...
describe("speech dictation",()=>{
 it("hydrates supported browsers from the same unsupported server snapshot",async()=>{const html=renderToString(<SpeechDictationControl onFinal={vi.fn()}/>);expect(html).toContain("speech-unavailable");const host=document.createElement("div");host.innerHTML=html;document.body.append(host);const consoleError=vi.spyOn(console,"error").mockImplementation(()=>undefined);const root=hydrateRoot(host,<SpeechDictationControl onFinal={vi.fn()}/>);await act(async()=>{});expect(host.querySelector("button[aria-label='Dicter mon raisonnement']")).toBeTruthy();expect(consoleError).not.toHaveBeenCalledWith(expect.stringContaining("Hydration failed"));root.unmount();host.remove();consoleError.mockRestore()});
