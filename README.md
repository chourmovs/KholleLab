...
3. Définissez exactement la configuration suivante (en remplaçant `PUBLIC_DOMAIN` par le nom visible dans le navigateur) :
   ...
   **Fusionner le code ne suffit pas : ces trois variables doivent aussi être configurées dans l'environnement Coolify.** En production, `AUTH_TRUSTED_ORIGINS` est obligatoire, ne se replie jamais sur `CORS_ORIGINS`, et une valeur limitée à `localhost` empêche le backend de démarrer. En développement et en test seulement, une valeur vide continue de se replier sur `CORS_ORIGINS`.

   Le domaine public doit inclure `https://`, ne contenir aucun chemin et employer le nom d'hôte externe exact. Une barre oblique finale est normalisée, mais il est recommandé de l'omettre. Les jokers et domaines suffixes ne sont pas acceptés. Le contrôle compare l'origine complète (schéma, hôte et port) : `http://PUBLIC_DOMAIN` et `https://PUBLIC_DOMAIN.evil.example` sont donc refusés.
