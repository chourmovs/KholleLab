...
from app.providers.llm import FakeLLMProvider,LocalLLMProvider
async def main(args):
    ...
    provider=FakeLLMProvider() if args.provider=="fake" else LocalLLMProvider()
...
p=argparse.ArgumentParser(); p.add_argument("--provider",choices=("fake","local"),default="fake"); p.add_argument("--level",choices=("seconde","premiere","terminale","maths-sup","maths-spe")); p.add_argument("--case"); p.add_argument("--quick",action="store_true"); p.add_argument("--json",nargs="?",const="artifacts/pedagogy-benchmark.json")
