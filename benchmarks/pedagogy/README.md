...
The deterministic harness runs at the requested help level and never sends reference rubrics or expected answers to the remote model. CI uses the fake provider to test plumbing only. Remote runs use `HuggingFaceProvider` with its interactive `FAST` role; stochastic model quality does not gate CI or deployment.
...
Run `python scripts/validate_benchmarks.py`, then `python scripts/bench_pedagogy.py --provider fake`. Use `--provider huggingface --family qwen --quick` on a remote-enabled deployment. Add `--json` to write the ignored `artifacts/pedagogy-benchmark.json` report.
