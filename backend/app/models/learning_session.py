...
class LearningSession(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_learning_sessions_learner_id", "learner_id"),
        UniqueConstraint("learner_id", "active_problem_key", name="uq_learning_sessions_learner_active_problem"),
    )

    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    problem_id: Mapped[str] = mapped_column(String(255), nullable=False)
    # NULL is reserved for preserved, unclaimed rows created before anonymous identity.
    learner_id: Mapped[uuid.UUID | None] = mapped_column(Uuid)
    ...
    active_problem_key: Mapped[str | None] = mapped_column(String(255))
