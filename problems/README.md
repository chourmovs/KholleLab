...
Les exercices paramétriques déterministes sont commités sous forme de bundles
par famille (`generated/*.yaml`, clé `problems`). Le dépôt valide chaque entrée
avec exactement le même schéma et la même API que les exercices éditoriaux.
Ne pas modifier ces bundles à la main : consulter `docs/problem-factory.md`,
lancer `python scripts/build_problem_corpus.py --write`, puis `--check`.
